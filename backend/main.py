...
#     # Fetch all users so the frontend dropdown can list them
...
# @app.get("/teams/")
# def read_teams(db: Session = Depends(database.get_db)):
#     # Add joinedload to fetch members relationship
#     return db.query(models.MaintenanceTeam).options(
#         joinedload(models.MaintenanceTeam.members)
#     ).all()
...
@app.get("/teams/")
def read_teams(db: Session = Depends(database.get_db)):
    # FIX: Include members so the Team List page works!
    return db.query(models.MaintenanceTeam).options(
        joinedload(models.MaintenanceTeam.members)
    ).all()
